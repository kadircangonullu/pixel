...
function openArtworkDesigner(){
  ...
  document.getElementById("artworkDesignerAnchor").textContent=`${province} · X:${selectedPixel.x} Y:${selectedPixel.y}`;designerColor=selectedTeam?.color||"#ff4b9b";designerErase=false;renderDesignerPalette();resetDesigner();
  // V21.6: once validation succeeds, hand off from Savaş Merkezi to the designer.
  closeTeamCenter();
  document.getElementById("artworkDesignerModal")?.classList.remove("hidden");
}
...
// V21.6 · leading fandom music.
// Direct audio_url tracks are preferred and play without YouTube's ad layer.
// YouTube remains a fallback for tracks that only have a youtube_video_id.
let leaderMusicEnabled=localStorage.getItem("fanverse_music_enabled")!=="0",leaderMusicTeam=null,leaderMusicTracks=[],leaderMusicIndex=0,leaderYTPlayer=null,leaderYTReady=false,leaderMusicGesture=false,leaderAudio=null;
async function loadLeaderMusic(teamId){if(!SUPABASE_ENABLED||!supabaseClient||!teamId)return[];const {data,error}=await supabaseClient.from("fandom_music_tracks").select("id,team_id,title,youtube_video_id,audio_url,position").eq("team_id",teamId).eq("enabled",true).order("position");if(error){console.warn("music playlist",error);return[]}return data||[];}
function currentLeaderTrack(){return leaderMusicTracks[leaderMusicIndex]||null;}
function renderLeaderMusicDock(){const team=teamById(leaderMusicTeam),current=currentLeaderTrack();const dot=document.getElementById("leaderMusicDot"),title=document.getElementById("leaderMusicTitle"),track=document.getElementById("leaderMusicTrack"),btn=document.getElementById("leaderMusicToggle");if(dot)dot.style.background=team?.color||"#94a3b8";if(title)title.textContent=team?`${team.name} önde`:"Lider bekleniyor";if(track)track.textContent=leaderMusicTracks.length?(current?.title||"Playlist"):`${team?.name||"Fandom"} için playlist henüz eklenmemiş`;if(btn)btn.textContent=leaderMusicEnabled?((leaderAudio&&!leaderAudio.paused)||(leaderYTPlayer&&leaderMusicGesture)?"DURDUR":"MÜZİĞİ BAŞLAT"):"MÜZİĞİ AÇ";}
function stopLeaderPlayers(){if(leaderAudio){leaderAudio.pause();}try{leaderYTPlayer?.pauseVideo?.();}catch{}}
function advanceLeaderTrack(){if(!leaderMusicTracks.length)return;leaderMusicIndex=(leaderMusicIndex+1)%leaderMusicTracks.length;playLeaderTrack();}
function ensureLeaderAudio(){if(leaderAudio)return leaderAudio;leaderAudio=new Audio();leaderAudio.preload="none";leaderAudio.addEventListener("ended",advanceLeaderTrack);leaderAudio.addEventListener("error",()=>{const t=currentLeaderTrack();if(t?.youtube_video_id)playLeaderYouTube(t.youtube_video_id);else{showToast("Ses dosyası oynatılamadı; playlistte sonraki parçaya geçiliyor.");advanceLeaderTrack();}});return leaderAudio;}
function playLeaderDirect(url){if(!url)return false;try{leaderYTPlayer?.pauseVideo?.();}catch{}const audio=ensureLeaderAudio();if(audio.src!==url)audio.src=url;audio.loop=leaderMusicTracks.length===1;audio.play().then(renderLeaderMusicDock).catch(err=>{console.warn("direct music playback",err);});return true;}
function playLeaderYouTube(vid){if(!vid||!window.YT?.Player)return false;if(leaderAudio)leaderAudio.pause();if(!leaderYTPlayer){leaderYTPlayer=new YT.Player("leaderYoutubePlayer",{height:"1",width:"1",videoId:vid,playerVars:{autoplay:1,controls:0,playsinline:1},events:{onReady:e=>{leaderYTReady=true;e.target.playVideo();renderLeaderMusicDock();},onStateChange:e=>{if(e.data===YT.PlayerState.ENDED)advanceLeaderTrack();}}});}else if(leaderYTReady){leaderYTPlayer.loadVideoById(vid);leaderYTPlayer.playVideo();}renderLeaderMusicDock();return true;}
async function syncLeaderMusic(){const leader=currentLeadingTeam();if(!leader||leader.id===leaderMusicTeam&&leaderMusicTracks.length)return;stopLeaderPlayers();leaderMusicTeam=leader.id;leaderMusicIndex=0;leaderMusicTracks=await loadLeaderMusic(leader.id);renderLeaderMusicDock();if(leaderMusicEnabled&&leaderMusicGesture)playLeaderTrack();}
function playLeaderTrack(){if(!leaderMusicEnabled||!leaderMusicGesture||!leaderMusicTracks.length)return;const track=currentLeaderTrack();if(track?.audio_url){playLeaderDirect(track.audio_url);return;}if(track?.youtube_video_id)playLeaderYouTube(track.youtube_video_id);renderLeaderMusicDock();}
window.onYouTubeIframeAPIReady=()=>{if(leaderMusicEnabled&&leaderMusicGesture&&!currentLeaderTrack()?.audio_url)playLeaderTrack();};
document.getElementById("leaderMusicToggle")?.addEventListener("click",()=>{leaderMusicGesture=true;const playingDirect=leaderAudio&&!leaderAudio.paused;if(leaderMusicEnabled&&(playingDirect||leaderYTPlayer)){leaderMusicEnabled=false;stopLeaderPlayers();}else{leaderMusicEnabled=true;playLeaderTrack();}localStorage.setItem("fanverse_music_enabled",leaderMusicEnabled?"1":"0");renderLeaderMusicDock();});
