...
const pageCopy={dashboard:['Genel Bakış','Sezon, kullanıcı ve moderasyon durumunu tek ekrandan yönet.'],seasons:['Sezonlar','Yeni sezon aç, tarihleri ve durumları düzenle.'],battlemap:['Savaş Motoru','81 il rasterını doğrula, hazırla ve server-side savaş durumunu yeniden kur.'],players:['Oyuncular','Hesapları incele, banla veya kalıcı olarak sil.'],roles:['Fandom Rolleri','Sınırlı Komutan ve Stratejist rollerini yönet.'],pixels:['Piksel Moderasyonu','Son yerleştirmeleri denetle ve uygunsuz pikselleri kaldır.'],history:['Geçmiş / Rollback','Koordinat ve kullanıcı geçmişini incele, gerektiğinde geri al.'],awards:['Sezon Ödülleri','Sezon sonu ödüllerini hesapla ve incele.'],zones:['Korumalı Alanlar','Artwork veya etkinlik alanlarını takım bazlı korumaya al.'],music:['Fandom Müzikleri','Lider fandomun arka plan playlistini yönet.'],logs:['Moderasyon Logu','Admin işlemlerinin denetim kaydını görüntüle.']};
document.querySelectorAll('.admin-nav').forEach(btn=>btn.onclick=()=>{
  ...
  ({dashboard:loadDashboard,seasons:loadSeasons,players:loadPlayers,roles:loadRoles,pixels:loadPixels,history:()=>{},awards:loadAwards,battlemap:loadBattleMapStatus,zones:loadZones,music:loadMusic,logs:loadLogs}[btn.dataset.tab])?.();
});
...
async function loadMusic(){if(!isAdminUser)return;const team=$('musicTeam')?.value||'crush';const {data,error}=await sb.from('fandom_music_tracks').select('*').eq('team_id',team).order('position');if(error){$('musicList').innerHTML=`<div class="list-item">${esc(error.message)}</div>`;return;}$('musicList').innerHTML=(data||[]).map(t=>`<div class="list-item"><div><strong>${esc(t.title)}</strong><small>${t.audio_url?'Doğrudan ses':'YouTube'} · ${esc(t.audio_url||t.youtube_video_id||'kaynak yok')} · sıra ${Number(t.position)}</small></div><div class="actions"><button data-music-delete="${t.id}" class="danger">SİL</button></div></div>`).join('')||'<div class="list-item">Bu fandom için parça yok.</div>';document.querySelectorAll('[data-music-delete]').forEach(b=>b.onclick=async()=>{const {error}=await sb.rpc('admin_delete_music_track',{p_track_id:b.dataset.musicDelete});if(error)toast(error.message);else{toast('Parça silindi.');loadMusic();}});}
$('musicTeam')?.addEventListener('change',loadMusic);$('refreshMusicBtn')?.addEventListener('click',loadMusic);$('addMusicTrackBtn')?.addEventListener('click',async()=>{const team=$('musicTeam').value,title=$('musicTitle').value.trim(),video=$('musicVideoId').value.trim(),audio=$('musicAudioUrl')?.value.trim()||'',position=Number($('musicPosition').value||1);const validVideo=!video||/^[A-Za-z0-9_-]{6,20}$/.test(video),validAudio=!audio||/^https:\/\//i.test(audio);if(!title||(!video&&!audio)||!validVideo||!validAudio){toast('Şarkı adı ve en az bir geçerli kaynak gir: HTTPS ses URL’si veya YouTube video ID.');return;}const {error}=await sb.rpc('admin_add_music_track',{p_team_id:team,p_title:title,p_youtube_video_id:video||null,p_position:position,p_audio_url:audio||null});if(error)toast(error.message);else{$('musicTitle').value='';$('musicVideoId').value='';if($('musicAudioUrl'))$('musicAudioUrl').value='';toast('Parça eklendi.');loadMusic();}});
